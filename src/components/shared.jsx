...
export function Header({ user: initialUser }) {
  ...
  useEffect(() => {
    ...
    // 1. Preferimos el username que ya viene enriquecido desde AuthContext
    if (user.username) {
      setUsername(user.username);
      return;
    }

    // 2. Fallback a user_metadata (por si el registro lo guardó ahí)
    if (user.user_metadata?.username) {
      setUsername(user.user_metadata.username);
      return;
    }

    // 3. Último recurso: consultar la API
    let cancelled = false;
    ...
    api.getUserProfile(user.id)
      ...
      .catch((err) => {
        if (!cancelled) {
          console.error('Error loading header username:', err);
        }
      });
  ...
  const profileUrl = username ? `/${username}` : '/settings';
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          {user ? (
            <div className="relative">
              ...
              >
                <span className="text-xs font-bold text-[#87ceeb]">{displayInitial}</span>
              </button>
